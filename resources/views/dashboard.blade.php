...
.animate-out { animation: fadeOut 0.3s ease-out; }
</style>
...
<div id="notifKritis" class="bg-white p-4 rounded shadow mt-6 animate-fade relative max-w-xl">
        ...
        <table class="w-full text-left border border-gray-200">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="p-2 border">
                        <span class="px-2 py-1 rounded bg-red-100 text-red-700 text-sm">Kritis</span>
                    </td>
                </tr>
                @endforeach
            </tbody>
...
<div class="bg-white p-4 rounded shadow mt-6">
    <h2 class="text-lg font-semibold mb-4">Status Stok Barang</h2>
    <div class="flex flex-col md:flex-row md:justify-between mb-4 gap-2">
        <div></div>
        <div class="flex items-center gap-2">
            <input type="text" id="searchInput" placeholder="Cari Barang..." class="pl-10 pr-3 py-2 border border-gray-300 rounded shadow-sm focus:outline-none w-48">
            <select id="filterStatus" class="pl-10 pr-3 py-2 border border-gray-300 rounded shadow-sm">
...
function renderChart(tahun) {
    ...
    chart = new Chart(ctx, {
        ...
        data: {
            labels: data.labels,
            datasets: [
                { label: 'Barang Masuk', data: data.masuk, borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.2)', tension: 0.4, fill: true },
                { label: 'Barang Keluar', data: data.keluar, borderColor: '#f97316', backgroundColor: 'rgba(249,115,22,0.2)', tension: 0.4, fill: true }
            ]
        },
...
function filterTable() {
    ...
    rows.forEach(row => {
        ...
        row.style.display = (status === 'semua' || rowStatus === status) && rowNama.includes(search) ? '' : 'none';
    });
}
