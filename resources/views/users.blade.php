...
@section('content')
                                ...
                                <div id="password-input-{{ $user->id }}" class="hidden">
                                    ...
                                    <div class="relative">
                                        ...
                                        <button type="button" onclick="togglePasswordEdit({{ $user->id }})"
                                            class="absolute right-2 top-2 text-gray-500 hover:text-blue-600 transition-colors">
                                            <svg id="eyeOpen-{{ $user->id }}" xmlns="http://www.w3.org/2000/svg"
                                            ...
                                            <svg id="eyeClosed-{{ $user->id }}" xmlns="http://www.w3.org/2000/svg"
                                                class="h-6 w-6 block" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                                    d="M2 12s4-5 10-5 10 5 10 5-4 5-10 5S2 12 2 12z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
    ...
    <script>
        ...
        function resetEditModal(id) {
            ...
            // Sembunyikan input password edit (checkbox default unchecked)
            const passwordWrapper = document.getElementById('password-input-' + id);
            if (passwordWrapper) {
                passwordWrapper.classList.add('hidden');
            }

            // Selalu paksa input passwordEdit jadi type password
            const passwordInput = modal.querySelector('input[name="passwordEdit"]');
            if (passwordInput) {
                passwordInput.type = 'password';
            }
        ...
        function togglePasswordInput(id) {
            ...
            const passwordWrapper = document.getElementById('password-input-' + id);
            const passwordInput = passwordWrapper.querySelector('input[name="passwordEdit"]');
            const eyeOpen = document.getElementById('eyeOpen-' + id);
            const eyeClosed = document.getElementById('eyeClosed-' + id);
