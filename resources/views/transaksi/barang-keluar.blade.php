...
@section('content')
    ...
    <div class="p-4 rounded mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <form method="GET" action="{{ route('barang-keluar.index') }}" id="filter-form" class="flex gap-4 flex-wrap">
            <div class="relative">
        ...
        <div id="modal-tambah-barang" onclick="handleBackdropClick(event)"
            ...
            <div class="bg-white rounded-lg w-full max-w-lg p-6 shadow-lg relative">
                ...
                <form action="{{ route('barang-keluar.add') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
            ...
            <tbody>
                @forelse ($wareOuts as $wareOut)
                    <tr class="border-b">
                        ...
                        <td class="px-4 py-2">{{ $wareOut->amount }}</td>
                        <td class="px-4 py-2 text-center space-x-2">
                            <button onclick="openEditModal({{ $wareOut->id }})"
                            ...
                            <button onclick="openDeleteModal({{ $wareOut->id }})"
                    ...
                    <div id="modal-edit-barang-{{ $wareOut->id }}" onclick="handleBackdropClickEdit(event)"
                        class="fixed inset-0 bg-transparent backdrop-blur-sm hidden justify-center items-center z-50">
                        <div class="bg-white rounded-lg w-full max-w-lg p-6 shadow-lg relative">
                            <button onclick="closeEditModal({{ $wareOut->id }})"
                                class="absolute top-3 right-3 text-gray-500 hover:text-red-500">
                                <img src="https://api.iconify.design/mdi/close.svg?color=gray" class="w-5 h-5" />
                            </button>

                            @if ($errors->getBag('edit_' . $wareOut->id)->any())
                                <div
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div>
                                            <strong class="font-bold">Oops!</strong>
                                            <ul class="mt-1 list-disc list-inside text-sm">
                                                @foreach ($errors->getBag('edit_' . $wareOut->id)->all() as $error)
                                                    <li>{{ $error }}</li>
                                                @endforeach
                                            </ul>
                                        </div>
                            ...
                            <form action="{{ route('barang-keluar.update', $wareOut->id) }}" method="POST"
                                class="space-y-4">
                                @csrf
                                @method('PUT')
                                    ...
                                    <select name="ware_id" required
                                        ...
                                        @foreach ($wares as $ware)
                                            <option value="{{ $ware->id }}"
                                                {{ old('ware_id', $wareOut->ware_id) == $ware->id ? 'selected' : '' }}>
                                                {{ $ware->ware_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="amount" value="{{ $wareOut->amount }}" min="0"
                                        required
                                        class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none">
                                </div>

                                <div class="flex justify-end gap-2 mt-2">
                                    <button type="button" onclick="closeEditModal({{ $wareOut->id }})"
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            @if ($errors->any())
                openModal();
            @endif
            @if (session('modal_target') === 'modal-tambah')
                openModal();
            @endif
            @foreach ($wareOuts as $wareOut)
                @if ($errors->getBag('edit_' . $wareOut->id)->any())
                    openEditModal({{ $wareOut->id }});
                @endif
            @endforeach
        });
